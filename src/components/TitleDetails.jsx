  ...
  const [showFullPlot, setShowFullPlot] = useState(false)
    ...
    <div className="md:h-5/6 bg-fixed bg-cover bg-center py-8 px-8 md:px-24" style={{backgroundImage: `url(${title.backdrop_path})`}}>
    	<div className={`w-full flex flex-col sm:flex-row items-center sm:items-start rounded-2xl p-2 shadow-xl backdrop-filter ${darkMode ? "backdrop-brightness-50" : "backdrop-brightness-75"} backdrop-blur-sm`}>
      	...
      	<div className={`flex-1 px-2 sm:px-8 py-4 space-y-3 ${darkMode ? "text-gray-200" : "text-white"}`}>
      		<a href={title.homepage}>
      			<h2 className="text-3xl font-bold text-gray-100">{title.name}</h2>
      		</a>
      		<div className="flex flex-wrap">
      			{title.genres.map(genre => (
      				<span key={genre.id} className="px-2 m-1 rounded-full border-2 border-gray-300">{genre.name}</span>
      			))}
      		</div>
      		<div>
            ...
            <div className="text-sm">
      		...
      		<div className="flex justify-center space-x-3 text-center">
      			<div>
      				...
      				<span className="rounded-full bg-gray-200 p-1.5 text-gray-800">
